...
def prompt(messages, model="gpt-4", temperature=0.5, max_tokens=None):
    headers = {
    ...
    if response.status_code == 200:
        ...
        return "// ERROR GENERATING GPT4 OUTPUT"
...
def remove_multiline_comments(input_filename):
    with open(input_filename, 'r') as infile:
        content = infile.read()
...
def add_header_sva_gen(content_without_comments, output_filename, header_filename, reg_string, name):
...
def add_header_autosva_annot_gen(content_without_comments, output_filename):
    # Load the content from the header file
    with open("AUTOSVA_GEN.v", 'r') as header_file:
        header_content = header_file.read()
...
def parse_args():
  ...
  parser.add_argument('-header', type=str, default='SVA_GEN.v', help='Path to the header file. Default is "SVA_GEN.v".')
...
def gen_tcl(dut_root,ft_path, dut_folder, src_list, include, dut_name, dut_name_ext, submodule_assert, submodule_assume):
    ...
    if (not os.path.exists(tcl_filename) or override_tool_script):
        ...
        tcl.write("set AUTOSVA_ROOT $env(AUTOSVA_ROOT)\n")
        ...
        tcl.write("elaborate -top " + dut_name + " -create_related_covers {witness precondition} -auto_hr_info\n")
...
def autosva_main():
    ...
    if API_KEY != "<YOUR_API_KEY>":
        # AutoSVA2: Create Prompt from RTL file
        base_filename = dut_path.rsplit('.', 1)[0]  # Removing the extension
        sva_prompt_file = "{}_PROMPT.v".format(base_filename)
        remove_verilog_comments(dut_path, sva_prompt_file)
        content_no_comments = remove_multiline_comments(sva_prompt_file)
        prompt_content = add_header_sva_gen(content_no_comments, sva_prompt_file, args.header, args.reg, dut_name)
        print("Done! Output written to {}".format(sva_prompt_file))
        # Prompt GPT4 to generate SVA code
        messages = [{"role": "user", "content": prompt_content}]
        sva_properties = prompt(messages)
        # print(sva_properties)
        prop.write("\n// *** GPT4-GENERATED SVA BATCH *** //\n")
        prop.write(sva_properties)
    ...
    autosva_prompt_file = "{}_AUTOSVA.v".format(base_filename)
    prompt_content = add_header_autosva_annot_gen(content_no_comments, autosva_prompt_file)
    # Prompt GPT4 to generate AutoSVA Annotations
    messages = [{"role": "user", "content": prompt_content}]
    autosva_annotations = prompt(messages)
    print(autosva_annotations)
